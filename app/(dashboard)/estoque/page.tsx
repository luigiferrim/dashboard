...
import { NewLotModal } from "@/components/estoque/new-lot-modal"
...
export default function EstoquePage() {
  ...
  const getDaysFromRoast = (roastDate: string | null | undefined) => {
    if (!roastDate) return null
    const days = Math.ceil((new Date().getTime() - new Date(roastDate).getTime()) / (1000 * 60 * 60 * 24))
    return days
  }
        ...
        <table className="w-full min-w-[800px]">
          ...
          <tbody>
            {filteredLots.map((lot) => {
              const daysOld = getDaysFromRoast(lot.roastDate)
              const isOld = daysOld !== null && daysOld > 60
